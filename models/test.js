...
testSchema.statics.findBySpecialization = function(specialization) {
    return this.find({ 
        specialization: { $in: Array.isArray(specialization) ? specialization : [specialization] }, 
        isActive: true 
    });
};
...
testSchema.pre('validate', function(next) {
    if (this.isTestSeriesSpecific && !this.testSeriesID) {
        const error = new Error('Test Series ID is required when test is test series specific');
        return next(error);
    }
    if (!this.isTestSeriesSpecific && this.testSeriesID) {
    ...
    if (this.passingMarks && this.passingMarks > this.overallMarks) {
        const error = new Error('Passing marks cannot be greater than overall marks');
        return next(error);
    }
